...
calcIndexByLayer <- function(layer){
  ...
  for(bn in bodyNames){
    for(ln in layerNames){
      if((tolower(layer) == ln) & (!is.na(idict[[bn]][[ln]]))){
...
calcCLOarea <- function(clo){
  fcl <- c()
  for (i in clo) {
    if(i < 0.5) fcl <- append(fcl, i*0.2 + 1)
    else fcl <- append(fcl, i*0.1 + 1.05)
  }
  fcl
}
...
calcSKINbloodflow <- function(err_cr, err_sk, height=1.72, weight=74.43, 
...
calcAVAbloodflow <- function(err_cr, err_sk, height=1.72, weight=74.43, 
                             equation="dubois", age=20, ci=2.59){
...
calcMETbasal <- function(height=1.72, weight=74.43, age=20, sex="male", 
...
Tcr <- function(bodyTemp){
  bodyTemp[index[["core"]]]
}
Tsk <- function(bodyTemp){
  bodyTemp[index[["skin"]]]
}
calcTsk <- function(bodyTemp, skinIndex){
  tskArray <- c()
  for (i in 1:length(skinIndex)) {
    tskArray <- append(tskArray, bodyTemp[skinIndex[i] +1])
  }
  tskArray
}
...
run <- function(dtime=60, passive=FALSE, output=TRUE, object){
  ...
  age <- object$age
  ci <- object$ci
  bmrEquation <- object$bmrEquation
  bsaEquation <- object$bsaEquation
  exOutput <- object$exOutput 
  bsar <- object$bsaRate
  bsa <- object$bsa
  bfbr <- object$bfbRate
  cdt <- object$cdt 
  cap <- object$cap 
  setptCr <- object$setptCr 
  setptSk <- object$setptSk 
  bodyTemp <- object$bodyTemp 
  to <- object$to
  ta <- object$ta 
  tr <- object$tr 
  rh <- object$rh 
  ...
  if(output){
    ...
    RES <- resSh + resLh
    THLsk <- shlsk + eSk
    dictout <- list(CycleTime = cycle, ModTime = ModTime, dt = dtime, 
  ...
  if(!is.null(exOutput)){
    ...
    BFms <- calcVIndex(bfMs, muscleVIndex)
    BFfat <- calcVIndex(bfFat, fatVIndex)
    Mbasecr <- mbase[[1]]
    Mbasems <- calcVIndex(mbase[[2]], muscleVIndex)
    Mbasefat <- calcVIndex(mbase[[3]], fatVIndex)
    Mbasesk <- mbase[[4]]
    Qms <- calcVIndex(qms, muscleVIndex)
    Qfat <- calcVIndex(qfat, fatVIndex)
    detailout <- list(Name = modelName, Height = height, Weight = weight,
                      ...
                      Mbasesk = Mbasesk, Mwork = mwork, Mshiv = mshiv[[1]], 
                      Mnst = mnst[[1]], Qcr = qcr, Qms = Qms, Qfat = Qfat, Qsk = qsk)
  ...
  if(!is.null(exOutput) && exOutput =="all" ){
    dictout <- append(dictout, detailout)
  } else if(is.vector(exOutput)){  # if exOut type is list
    outkeys <- names(detailout)
    for (i in exOutput) {
      if(i %in% outkeys){
        dictout[i] <- detailout[i]
      }
    }
  }
  
  # internal variables
  updatedVariables <- list(height = height, weight = weight, fat = fat, sex = sex,
                           age = age, ci = ci, bmrEquation = bmrEquation, 
                           bsaEquation = bsaEquation, exOutput = exOutput, 
                           bsaRate = bsar, bsa = bsa, bfbRate = bfbr, cdt = cdt, cap = cap, 
                           setptCr = setptCr, setptSk = setptSk, bodyTemp = bodyTemp, to = to, ta = ta, tr = tr, rh = rh, va = va, clo = clo, iclo = iclo, 
                           par = par, posture = posture, hc = hc, hr = hr, exQ = exQ, cycle = cycle, modelName = modelName, options = options, 
                           history = history)
  return(list(dictout = dictout, updatedVariables = updatedVariables))
}
