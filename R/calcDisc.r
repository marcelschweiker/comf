...
#'
#' @aliases calcDisc Disc disc calcdisc
#' @description
...
#' Disc <- sapply(seq(maxLength), function(x) {
#'   calcDisc(ta[x], tr[x], vel[x], rh[x])
#' })
calcDisc <- function(ta, tr, vel, rh, clo = .5, met = 1, wme = 0, pb = 760,
                     ltime = 60, ht = 171, wt = 70, tu = 40, obj = "set",
                     csw = 170, cdil = 120, cstr = .5) {
  calc2Node(
    ta = ta, tr = tr, vel = vel, rh = rh, clo = clo, met = met, wme = wme, NULL,
