...
#'
#' @aliases calcSkinWettedness skinWettedness wettedness skin
#' @description
#' \code{calcSkinWettedness} calculates Skin Wettedness based on the 2-Node-Model
#' by Gagge et al.
#'
#' @usage
...
#' @param varOut a string value "skinWet" to report value of skin wettedness
#'
#' @details
...
#'
#' @return
#' \code{calcSkinWettedness} returns the Skin Wettedness
#'
#' @note
...
#' skinWet <- sapply(seq(maxLength), function(x) {
#'   calcSkinWettedness(ta[x], tr[x], vel[x], rh[x])
#' })
calcSkinWettedness <- function(ta, tr, vel, rh, clo = .5, met = 1, wme = 0,
                               pb = 760, ltime = 60, ht = 171, wt = 70, tu = 40,
                               obj = "set", csw = 170, cdil = 120, cstr = .5,
                               varOut = "skinWet") {
  as.numeric(calc2Node(ta, tr, vel, rh, clo, met, wme, NULL, pb, ltime, ht, wt, tu, obj, csw, cdil, cstr, varOut = "skinWet"))
}
