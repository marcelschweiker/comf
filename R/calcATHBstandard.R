...
#' 
#' @usage calcATHBstandard(trm, ta, tr, vel, rh, met, clo = 0.5, cloAdapt = TRUE)
#'
...
#' @param cloAdapt - a boolean value with TRUE as default that adapted clothing is used
...
#'
#' @details By default (with cloAdapt = TRUE), the clothing level will be calculated based on trm and any input to clo ignored.
#' @details If users want to use actual clothing insulation level, set cloAdapt to FALSE and define clo.
#'
#' @references 
...
calcATHBstandard <- function(trm, ta, tr, vel, rh, met, clo = 0.5, cloAdapt = TRUE){
