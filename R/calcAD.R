...
#' @aliases ankledraft
#' @aliases ad
#' @usage calcAD(ta, tr, vel, rh, clo, met, v_ankle)
#' @usage ad(ta, tr, vel, rh, clo, met, v_ankle)
#' @usage ankledraft(ta, tr, vel, rh, clo, met, v_ankle)
...
#' @param v_ankle air speed at the 0.1 m (4 in.) above the floor [m/s]
...
#' @examples calcAD(25,25,0.2,50,0.5,1.2,0.3)
#' @author Shoaib Sarwar
#' @export



calcAD <- function(ta,tr,vel,rh,clo,met,v_ankle){



  tsv = calcPMV(ta=ta, tr=tr, vel=vel, rh=rh, clo=clo, met=met)

  ppdval = (exp(-2.58 + 3.05 * v_ankle - 1.06 * tsv)/(1 + exp(-2.58 + 3.05 * v_ankle -1.06 * tsv))) * 100
