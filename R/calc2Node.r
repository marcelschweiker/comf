...
calc2Node <- function(ta, tr, vel, rh, clo = .5, met = 1, wme = 0, sa = NULL, pb = 760,
  ...
  fnerre <- function(x, hsk, hd, tsk, w, he, pssk) {
    hsk - (hd * (tsk - x)) - w * he.s * (pssk - .5 * fnsvp(x))
  }
  fnerrs <- function(x, hsk, hd.s, tsk, w, he.s, pssk) {
    hsk - hd.s * (tsk - x) - w * he.s * (pssk - .5 * fnsvp(x))
  }
